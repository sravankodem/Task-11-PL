...
from .models import User, Bus, Book,Emp 
...
def insert1(request):
    ...
    if request.method=="POST":
        ...
        com=request.POST['commission']
        dept=request.POST['dept']
        print(empid,hiredate,salary,com,dept)
        ins=Emp(empid=empid,hiredate=hiredate,salary=salary,commission=com,deptname=dept)
        ins.save()
        print("Data saved sucessfully")
    return render(request, 'myapp/inser_2.html',{'error':error})
def insert2(request):
    q = Emp.objects.all().order_by('-salary')
    Empid=q[0].empid
    Salary=q[0].salary
    Commission=q[0].commission
    Department=q[0].deptname
    print(Empid,Salary,Commission,Department)
    return HttpResponse("Check Terminal for results")
